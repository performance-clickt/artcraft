...
4. [Install nx](https://nx.dev/docs/getting-started/installation) — **macOS and Linux only**, and at the version the workspace pins (`21.2.3` in `frontend/package.json`): `npm install -g nx@21.2.3`. A newer global CLI drives a v21 workspace and produces plugin/daemon version-skew errors. The global install is needed because `./script/artcraft/unix_frontend_dev.sh` calls bare `nx`, which is not resolved from `node_modules/.bin` when run from a shell script. On Windows `windows_frontend_dev.ps1` runs `npx nx`, which uses the pinned local version, so no global install is required.
