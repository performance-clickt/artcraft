...
4. No global `nx` install is required. Both `./script/artcraft/unix_frontend_dev.sh` and `windows_frontend_dev.ps1` run `npx nx`, which resolves the workspace-pinned version (`21.2.3` in `frontend/package.json`) from `frontend/node_modules/.bin` after `npm install`. If you have a global `nx` on PATH from prior setup, it's unused by these scripts and safe to leave (or remove with `npm uninstall -g nx`).
